...
var arccore = require('arccore'); // Create a filter object via the filter factory.


var factoryResponse = arccore.filter.create({
  operationID: 'demo',
  // pick a random 22-char identifier
  ...
  bodyFunction: function bodyFunction(input) {
    var result;
    if (input !== "forceImplementationBug") result = input.length;else result = {
...
var exampleFilter = factoryResponse.result; // Test good and bad input.

var responses = {
  badInput: exampleFilter.request({
    message: "Not what the filter expects"
  }),
